...
class MongoTest(xmlrpc.XMLRPC):
    ...
    """
        ...
        Use of this methods:
            ...
            s.asyncEnsureIndex(name_db, name_col, key, num)
            ...
            s.ImportByMongo(type)
                Import data csv or json file by mongo import command. type='csv' or 'json'

            s.asyncInsert(name_db, name_col, number)
                Single inserting

            s.asyncBulk(name_db, name_col, number)
    ...
    def xmlrpc_asyncAddRecord(self, name_db, name_col, val):
        ...
        """
        log.msg(">>>call indexBulk to Mongo")
        d = deferToThread(self.xmlrpc_addRecord, name_db, name_col, val)
        return d
